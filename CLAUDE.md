...
### Development
```bash
# Start development server (recommended - includes queue, logs, vite)
composer dev

# Manual start (alternative)
php artisan serve

# Queue worker (required for async jobs like email, notifications)
...
# Clear test data (上线前使用)
php artisan test-data:clear
php artisan test-data:clear --force  # Skip confirmation
...
```bash
# Automated upgrade from Laravel 6 version
php artisan dujiaoka:upgrade --host=localhost --database=old_db --username=root --password=pass --old-path=/path/to/old
```
...
- **Access**: `/admin` route
- **Structure**: Resources follow Filament's convention - each resource has:
  - Resource class (e.g., `GoodsResource.php`)
  - `Tables/` - Table definitions
  - `Schemas/` - Form schemas
  - `Pages/` - CRUD pages (List, Create, Edit)
- **Widgets**: Dashboard widgets in `app/Filament/Widgets/`

### Core Domain Models
- **Goods** (`app/Models/Goods.php`) - Products/digital goods
- **GoodsGroup** - Product categories
- **Carmis** - Card/key inventory (卡密 = cards/keys)
- **Order** - Order management with status workflow
- **Coupon** - Discount coupons with goods relationships
- **Pay** - Payment gateway configurations
- **Emailtpl** - Email template management

### Service Layer Pattern
Services in `app/Service/` handle business logic:
- **OrderService** - Order validation and creation
- **OrderProcessService** - Order lifecycle (pending → paid → delivered)
- **GoodsService** - Product inventory management
- **CarmisService** - Card/key inventory handling
- **CouponService** - Coupon validation and usage
...
- **MailSend** - Email notifications
- **OrderExpired** - Order expiration handling
- **TelegramPush**, **ServerJiang**, **BarkPush**, **WorkWeiXinPush** - Various notification channels
- **ApiHook** - Webhook callbacks
- **CouponBack** - Coupon refund on order failure

**Important**: Queue worker MUST be running for notifications and order processing.
...
- `picture_ulr($file)` - Generate image URLs with fallback

### Middleware
Custom middleware in `app/Http/Middleware/`:
- **DujiaoBoot** - System initialization
- **InstallCheck** - Installation verification
- **PayGateWay** - Payment gateway validation
- **DujiaoSystem** - System-level checks

### Routes Organization
- `routes/web.php` - Loads all route files from `routes/common/`
- `routes/common/web.php` - Frontend routes
...
**Pint configuration** (`pint.json`) enforces:
- **Strict types**: `declare(strict_types=1);` required in all PHP files
- **File header**: PHPDoc comment "This file is part of dujiaoka next server projects."
- Location: Header must appear after `declare(strict_types=1);`
- Preset: Laravel standards

When creating new PHP files, always include:
```php
<?php
...
1. Create controller in `app/Http/Controllers/Pay/`
2. Add route in `routes/common/pay.php`
3. Create corresponding model record in `pays` table
4. Implement standard payment flow (notify, return, query methods)

### Working with Orders
- Order creation flows through `OrderService::validation()` → `OrderProcessService::createOrder()`
- Status transitions handled by `OrderProcessService` methods
- Events: `OrderUpdated` event triggers notifications
- Always dispatch jobs for notifications (don't process synchronously)

### Filament Resource Development
Follow the established pattern:
- Resource file defines table and form
- Separate `Tables/` and `Schemas/` files for organization
- Use `Pages/` for custom CRUD logic
- Register resources in `AdminPanelProvider`

### Asset Compilation
- Vite configuration in `vite.config.js`
- Tailwind CSS 4 with `@tailwindcss/vite` plugin
- Frontend assets served via Vite during development
- Build for production before deployment

### Testing with Pest
- Configuration in `phpunit.xml`
- Uses SQLite in-memory for tests
- Feature tests for integration, Unit tests for isolated logic
- Run queue in sync mode during tests
